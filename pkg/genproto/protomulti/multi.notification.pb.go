// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v3.15.8
// source: protomulti/multi.notification.proto

package protomulti

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type MultiLobbyNotificationType int32

const (
	MultiLobbyNotificationType_LOBBY_NOTIFY_NONE                    MultiLobbyNotificationType = 0
	MultiLobbyNotificationType_LOBBY_NOTIFY_PARTICIPANT_JOIN        MultiLobbyNotificationType = 100
	MultiLobbyNotificationType_LOBBY_NOTIFY_PARTICIPANT_LEAVE       MultiLobbyNotificationType = 200
	MultiLobbyNotificationType_LOBBY_NOTIFY_PARTICIPANT_READY       MultiLobbyNotificationType = 300
	MultiLobbyNotificationType_LOBBY_NOTIFY_PARTICIPANT_UNREADY     MultiLobbyNotificationType = 400
	MultiLobbyNotificationType_LOBBY_NOTIFY_STAMP_SEND              MultiLobbyNotificationType = 500
	MultiLobbyNotificationType_LOBBY_NOTIFY_PARTICIPANT_DECK_CHANGE MultiLobbyNotificationType = 600
	MultiLobbyNotificationType_LOBBY_NOTIFY_CANCEL                  MultiLobbyNotificationType = 700
	MultiLobbyNotificationType_LOBBY_NOTIFY_START                   MultiLobbyNotificationType = 800
	MultiLobbyNotificationType_LOBBY_NOTIFY_READY                   MultiLobbyNotificationType = 900
)

// Enum value maps for MultiLobbyNotificationType.
var (
	MultiLobbyNotificationType_name = map[int32]string{
		0:   "LOBBY_NOTIFY_NONE",
		100: "LOBBY_NOTIFY_PARTICIPANT_JOIN",
		200: "LOBBY_NOTIFY_PARTICIPANT_LEAVE",
		300: "LOBBY_NOTIFY_PARTICIPANT_READY",
		400: "LOBBY_NOTIFY_PARTICIPANT_UNREADY",
		500: "LOBBY_NOTIFY_STAMP_SEND",
		600: "LOBBY_NOTIFY_PARTICIPANT_DECK_CHANGE",
		700: "LOBBY_NOTIFY_CANCEL",
		800: "LOBBY_NOTIFY_START",
		900: "LOBBY_NOTIFY_READY",
	}
	MultiLobbyNotificationType_value = map[string]int32{
		"LOBBY_NOTIFY_NONE":                    0,
		"LOBBY_NOTIFY_PARTICIPANT_JOIN":        100,
		"LOBBY_NOTIFY_PARTICIPANT_LEAVE":       200,
		"LOBBY_NOTIFY_PARTICIPANT_READY":       300,
		"LOBBY_NOTIFY_PARTICIPANT_UNREADY":     400,
		"LOBBY_NOTIFY_STAMP_SEND":              500,
		"LOBBY_NOTIFY_PARTICIPANT_DECK_CHANGE": 600,
		"LOBBY_NOTIFY_CANCEL":                  700,
		"LOBBY_NOTIFY_START":                   800,
		"LOBBY_NOTIFY_READY":                   900,
	}
)

func (x MultiLobbyNotificationType) Enum() *MultiLobbyNotificationType {
	p := new(MultiLobbyNotificationType)
	*p = x
	return p
}

func (x MultiLobbyNotificationType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (MultiLobbyNotificationType) Descriptor() protoreflect.EnumDescriptor {
	return file_protomulti_multi_notification_proto_enumTypes[0].Descriptor()
}

func (MultiLobbyNotificationType) Type() protoreflect.EnumType {
	return &file_protomulti_multi_notification_proto_enumTypes[0]
}

func (x MultiLobbyNotificationType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use MultiLobbyNotificationType.Descriptor instead.
func (MultiLobbyNotificationType) EnumDescriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{0}
}

type MultiGameNotificationType int32

const (
	MultiGameNotificationType_GAME_NOTIFY_NONE                       MultiGameNotificationType = 0
	MultiGameNotificationType_GAME_NOTIFY_START                      MultiGameNotificationType = 100
	MultiGameNotificationType_GAME_NOTIFY_READY                      MultiGameNotificationType = 200
	MultiGameNotificationType_GAME_NOTIFY_PLAYER_READY               MultiGameNotificationType = 250
	MultiGameNotificationType_GAME_NOTIFY_PLAYER_REMOVE              MultiGameNotificationType = 275
	MultiGameNotificationType_GAME_NOTIFY_ENQUEUE_ACTION             MultiGameNotificationType = 1500
	MultiGameNotificationType_GAME_NOTIFY_DEQUEUE_ACTION             MultiGameNotificationType = 1600
	MultiGameNotificationType_GAME_NOTIFY_LOCK_ACTION                MultiGameNotificationType = 1700
	MultiGameNotificationType_GAME_NOTIFY_QUEUE_CONFIRM              MultiGameNotificationType = 1800
	MultiGameNotificationType_GAME_NOTIFY_HP_CONSENSUS               MultiGameNotificationType = 2000
	MultiGameNotificationType_GAME_NOTIFY_PLAYER_DISCONNECT          MultiGameNotificationType = 2100
	MultiGameNotificationType_GAME_NOTIFY_PLAYER_RECONNECT           MultiGameNotificationType = 2200
	MultiGameNotificationType_GAME_NOTIFY_GAME_SYNC                  MultiGameNotificationType = 2300
	MultiGameNotificationType_GAME_NOTIFY_PLAYER_DEATH               MultiGameNotificationType = 2400
	MultiGameNotificationType_GAME_NOTIFY_PLAYER_REVIVE              MultiGameNotificationType = 2500
	MultiGameNotificationType_GAME_NOTIFY_PLAYER_REVIVE_CLAIM        MultiGameNotificationType = 2600
	MultiGameNotificationType_GAME_NOTIFY_PLAYER_REVIVE_CLAIM_EXPIRE MultiGameNotificationType = 2700
	MultiGameNotificationType_GAME_NOTIFY_END                        MultiGameNotificationType = 8888
)

// Enum value maps for MultiGameNotificationType.
var (
	MultiGameNotificationType_name = map[int32]string{
		0:    "GAME_NOTIFY_NONE",
		100:  "GAME_NOTIFY_START",
		200:  "GAME_NOTIFY_READY",
		250:  "GAME_NOTIFY_PLAYER_READY",
		275:  "GAME_NOTIFY_PLAYER_REMOVE",
		1500: "GAME_NOTIFY_ENQUEUE_ACTION",
		1600: "GAME_NOTIFY_DEQUEUE_ACTION",
		1700: "GAME_NOTIFY_LOCK_ACTION",
		1800: "GAME_NOTIFY_QUEUE_CONFIRM",
		2000: "GAME_NOTIFY_HP_CONSENSUS",
		2100: "GAME_NOTIFY_PLAYER_DISCONNECT",
		2200: "GAME_NOTIFY_PLAYER_RECONNECT",
		2300: "GAME_NOTIFY_GAME_SYNC",
		2400: "GAME_NOTIFY_PLAYER_DEATH",
		2500: "GAME_NOTIFY_PLAYER_REVIVE",
		2600: "GAME_NOTIFY_PLAYER_REVIVE_CLAIM",
		2700: "GAME_NOTIFY_PLAYER_REVIVE_CLAIM_EXPIRE",
		8888: "GAME_NOTIFY_END",
	}
	MultiGameNotificationType_value = map[string]int32{
		"GAME_NOTIFY_NONE":                       0,
		"GAME_NOTIFY_START":                      100,
		"GAME_NOTIFY_READY":                      200,
		"GAME_NOTIFY_PLAYER_READY":               250,
		"GAME_NOTIFY_PLAYER_REMOVE":              275,
		"GAME_NOTIFY_ENQUEUE_ACTION":             1500,
		"GAME_NOTIFY_DEQUEUE_ACTION":             1600,
		"GAME_NOTIFY_LOCK_ACTION":                1700,
		"GAME_NOTIFY_QUEUE_CONFIRM":              1800,
		"GAME_NOTIFY_HP_CONSENSUS":               2000,
		"GAME_NOTIFY_PLAYER_DISCONNECT":          2100,
		"GAME_NOTIFY_PLAYER_RECONNECT":           2200,
		"GAME_NOTIFY_GAME_SYNC":                  2300,
		"GAME_NOTIFY_PLAYER_DEATH":               2400,
		"GAME_NOTIFY_PLAYER_REVIVE":              2500,
		"GAME_NOTIFY_PLAYER_REVIVE_CLAIM":        2600,
		"GAME_NOTIFY_PLAYER_REVIVE_CLAIM_EXPIRE": 2700,
		"GAME_NOTIFY_END":                        8888,
	}
)

func (x MultiGameNotificationType) Enum() *MultiGameNotificationType {
	p := new(MultiGameNotificationType)
	*p = x
	return p
}

func (x MultiGameNotificationType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (MultiGameNotificationType) Descriptor() protoreflect.EnumDescriptor {
	return file_protomulti_multi_notification_proto_enumTypes[1].Descriptor()
}

func (MultiGameNotificationType) Type() protoreflect.EnumType {
	return &file_protomulti_multi_notification_proto_enumTypes[1]
}

func (x MultiGameNotificationType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use MultiGameNotificationType.Descriptor instead.
func (MultiGameNotificationType) EnumDescriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{1}
}

type LobbyCancelNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	LobbyId       string                 `protobuf:"bytes,1,opt,name=lobby_id,json=lobbyId,proto3" json:"lobby_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LobbyCancelNotification) Reset() {
	*x = LobbyCancelNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LobbyCancelNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LobbyCancelNotification) ProtoMessage() {}

func (x *LobbyCancelNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LobbyCancelNotification.ProtoReflect.Descriptor instead.
func (*LobbyCancelNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{0}
}

func (x *LobbyCancelNotification) GetLobbyId() string {
	if x != nil {
		return x.LobbyId
	}
	return ""
}

type LobbyStartNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	LobbyId       string                 `protobuf:"bytes,1,opt,name=lobby_id,json=lobbyId,proto3" json:"lobby_id,omitempty"`
	GameId        string                 `protobuf:"bytes,2,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LobbyStartNotification) Reset() {
	*x = LobbyStartNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LobbyStartNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LobbyStartNotification) ProtoMessage() {}

func (x *LobbyStartNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LobbyStartNotification.ProtoReflect.Descriptor instead.
func (*LobbyStartNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{1}
}

func (x *LobbyStartNotification) GetLobbyId() string {
	if x != nil {
		return x.LobbyId
	}
	return ""
}

func (x *LobbyStartNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

type LobbyReadyNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	LobbyId       string                 `protobuf:"bytes,1,opt,name=lobby_id,json=lobbyId,proto3" json:"lobby_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LobbyReadyNotification) Reset() {
	*x = LobbyReadyNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LobbyReadyNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LobbyReadyNotification) ProtoMessage() {}

func (x *LobbyReadyNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LobbyReadyNotification.ProtoReflect.Descriptor instead.
func (*LobbyReadyNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{2}
}

func (x *LobbyReadyNotification) GetLobbyId() string {
	if x != nil {
		return x.LobbyId
	}
	return ""
}

type ParticipantJoinNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	LobbyId       string                 `protobuf:"bytes,1,opt,name=lobby_id,json=lobbyId,proto3" json:"lobby_id,omitempty"`
	PlayerId      string                 `protobuf:"bytes,2,opt,name=player_id,json=playerId,proto3" json:"player_id,omitempty"`
	DeckIndex     int32                  `protobuf:"varint,3,opt,name=deck_index,json=deckIndex,proto3" json:"deck_index,omitempty"`
	PlayerSlot    int32                  `protobuf:"varint,4,opt,name=player_slot,json=playerSlot,proto3" json:"player_slot,omitempty"`
	Player        *ProtoLobbyPlayer      `protobuf:"bytes,5,opt,name=player,proto3" json:"player,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ParticipantJoinNotification) Reset() {
	*x = ParticipantJoinNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ParticipantJoinNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ParticipantJoinNotification) ProtoMessage() {}

func (x *ParticipantJoinNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ParticipantJoinNotification.ProtoReflect.Descriptor instead.
func (*ParticipantJoinNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{3}
}

func (x *ParticipantJoinNotification) GetLobbyId() string {
	if x != nil {
		return x.LobbyId
	}
	return ""
}

func (x *ParticipantJoinNotification) GetPlayerId() string {
	if x != nil {
		return x.PlayerId
	}
	return ""
}

func (x *ParticipantJoinNotification) GetDeckIndex() int32 {
	if x != nil {
		return x.DeckIndex
	}
	return 0
}

func (x *ParticipantJoinNotification) GetPlayerSlot() int32 {
	if x != nil {
		return x.PlayerSlot
	}
	return 0
}

func (x *ParticipantJoinNotification) GetPlayer() *ProtoLobbyPlayer {
	if x != nil {
		return x.Player
	}
	return nil
}

type ParticipantLeaveNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	LobbyId       string                 `protobuf:"bytes,1,opt,name=lobby_id,json=lobbyId,proto3" json:"lobby_id,omitempty"`
	PlayerId      string                 `protobuf:"bytes,2,opt,name=player_id,json=playerId,proto3" json:"player_id,omitempty"`
	PlayerSlot    int32                  `protobuf:"varint,3,opt,name=player_slot,json=playerSlot,proto3" json:"player_slot,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ParticipantLeaveNotification) Reset() {
	*x = ParticipantLeaveNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ParticipantLeaveNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ParticipantLeaveNotification) ProtoMessage() {}

func (x *ParticipantLeaveNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ParticipantLeaveNotification.ProtoReflect.Descriptor instead.
func (*ParticipantLeaveNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{4}
}

func (x *ParticipantLeaveNotification) GetLobbyId() string {
	if x != nil {
		return x.LobbyId
	}
	return ""
}

func (x *ParticipantLeaveNotification) GetPlayerId() string {
	if x != nil {
		return x.PlayerId
	}
	return ""
}

func (x *ParticipantLeaveNotification) GetPlayerSlot() int32 {
	if x != nil {
		return x.PlayerSlot
	}
	return 0
}

type ParticipantReadyNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	LobbyId       string                 `protobuf:"bytes,1,opt,name=lobby_id,json=lobbyId,proto3" json:"lobby_id,omitempty"`
	PlayerSlot    int32                  `protobuf:"varint,3,opt,name=player_slot,json=playerSlot,proto3" json:"player_slot,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ParticipantReadyNotification) Reset() {
	*x = ParticipantReadyNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ParticipantReadyNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ParticipantReadyNotification) ProtoMessage() {}

func (x *ParticipantReadyNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ParticipantReadyNotification.ProtoReflect.Descriptor instead.
func (*ParticipantReadyNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{5}
}

func (x *ParticipantReadyNotification) GetLobbyId() string {
	if x != nil {
		return x.LobbyId
	}
	return ""
}

func (x *ParticipantReadyNotification) GetPlayerSlot() int32 {
	if x != nil {
		return x.PlayerSlot
	}
	return 0
}

type ParticipantUnreadyNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	LobbyId       string                 `protobuf:"bytes,1,opt,name=lobby_id,json=lobbyId,proto3" json:"lobby_id,omitempty"`
	PartySlot     int32                  `protobuf:"varint,2,opt,name=party_slot,json=partySlot,proto3" json:"party_slot,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ParticipantUnreadyNotification) Reset() {
	*x = ParticipantUnreadyNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ParticipantUnreadyNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ParticipantUnreadyNotification) ProtoMessage() {}

func (x *ParticipantUnreadyNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ParticipantUnreadyNotification.ProtoReflect.Descriptor instead.
func (*ParticipantUnreadyNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{6}
}

func (x *ParticipantUnreadyNotification) GetLobbyId() string {
	if x != nil {
		return x.LobbyId
	}
	return ""
}

func (x *ParticipantUnreadyNotification) GetPartySlot() int32 {
	if x != nil {
		return x.PartySlot
	}
	return 0
}

type ParticipantDeckChangeNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	LobbyId       string                 `protobuf:"bytes,1,opt,name=lobby_id,json=lobbyId,proto3" json:"lobby_id,omitempty"`
	PlayerSlot    int32                  `protobuf:"varint,3,opt,name=player_slot,json=playerSlot,proto3" json:"player_slot,omitempty"`
	Player        *ProtoLobbyPlayer      `protobuf:"bytes,4,opt,name=player,proto3" json:"player,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ParticipantDeckChangeNotification) Reset() {
	*x = ParticipantDeckChangeNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ParticipantDeckChangeNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ParticipantDeckChangeNotification) ProtoMessage() {}

func (x *ParticipantDeckChangeNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ParticipantDeckChangeNotification.ProtoReflect.Descriptor instead.
func (*ParticipantDeckChangeNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{7}
}

func (x *ParticipantDeckChangeNotification) GetLobbyId() string {
	if x != nil {
		return x.LobbyId
	}
	return ""
}

func (x *ParticipantDeckChangeNotification) GetPlayerSlot() int32 {
	if x != nil {
		return x.PlayerSlot
	}
	return 0
}

func (x *ParticipantDeckChangeNotification) GetPlayer() *ProtoLobbyPlayer {
	if x != nil {
		return x.Player
	}
	return nil
}

type StampSendNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	LobbyId       string                 `protobuf:"bytes,1,opt,name=lobby_id,json=lobbyId,proto3" json:"lobby_id,omitempty"`
	StampId       string                 `protobuf:"bytes,2,opt,name=stamp_id,json=stampId,proto3" json:"stamp_id,omitempty"`
	PartySlot     int32                  `protobuf:"varint,3,opt,name=party_slot,json=partySlot,proto3" json:"party_slot,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StampSendNotification) Reset() {
	*x = StampSendNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StampSendNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StampSendNotification) ProtoMessage() {}

func (x *StampSendNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StampSendNotification.ProtoReflect.Descriptor instead.
func (*StampSendNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{8}
}

func (x *StampSendNotification) GetLobbyId() string {
	if x != nil {
		return x.LobbyId
	}
	return ""
}

func (x *StampSendNotification) GetStampId() string {
	if x != nil {
		return x.StampId
	}
	return ""
}

func (x *StampSendNotification) GetPartySlot() int32 {
	if x != nil {
		return x.PartySlot
	}
	return 0
}

type GameStartNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	GameId        string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GameStartNotification) Reset() {
	*x = GameStartNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GameStartNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GameStartNotification) ProtoMessage() {}

func (x *GameStartNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GameStartNotification.ProtoReflect.Descriptor instead.
func (*GameStartNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{9}
}

func (x *GameStartNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

type GameEndNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	GameId        string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GameEndNotification) Reset() {
	*x = GameEndNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GameEndNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GameEndNotification) ProtoMessage() {}

func (x *GameEndNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GameEndNotification.ProtoReflect.Descriptor instead.
func (*GameEndNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{10}
}

func (x *GameEndNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

type GamePlayerReadyNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	GameId        string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	PartyIndex    int32                  `protobuf:"varint,2,opt,name=party_index,json=partyIndex,proto3" json:"party_index,omitempty"`
	PlayerIndex   int32                  `protobuf:"varint,3,opt,name=player_index,json=playerIndex,proto3" json:"player_index,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GamePlayerReadyNotification) Reset() {
	*x = GamePlayerReadyNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GamePlayerReadyNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GamePlayerReadyNotification) ProtoMessage() {}

func (x *GamePlayerReadyNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GamePlayerReadyNotification.ProtoReflect.Descriptor instead.
func (*GamePlayerReadyNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{11}
}

func (x *GamePlayerReadyNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

func (x *GamePlayerReadyNotification) GetPartyIndex() int32 {
	if x != nil {
		return x.PartyIndex
	}
	return 0
}

func (x *GamePlayerReadyNotification) GetPlayerIndex() int32 {
	if x != nil {
		return x.PlayerIndex
	}
	return 0
}

type GameReadyNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	GameId        string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GameReadyNotification) Reset() {
	*x = GameReadyNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GameReadyNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GameReadyNotification) ProtoMessage() {}

func (x *GameReadyNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GameReadyNotification.ProtoReflect.Descriptor instead.
func (*GameReadyNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{12}
}

func (x *GameReadyNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

type GameHPSyncNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	GameId        string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	Enemies       []*ProtoGameEnemyHP    `protobuf:"bytes,2,rep,name=enemies,proto3" json:"enemies,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GameHPSyncNotification) Reset() {
	*x = GameHPSyncNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GameHPSyncNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GameHPSyncNotification) ProtoMessage() {}

func (x *GameHPSyncNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GameHPSyncNotification.ProtoReflect.Descriptor instead.
func (*GameHPSyncNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{13}
}

func (x *GameHPSyncNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

func (x *GameHPSyncNotification) GetEnemies() []*ProtoGameEnemyHP {
	if x != nil {
		return x.Enemies
	}
	return nil
}

type GameEnqueueActionNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	GameId        string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	PartyIndex    int32                  `protobuf:"varint,2,opt,name=party_index,json=partyIndex,proto3" json:"party_index,omitempty"`
	PlayerIndex   int32                  `protobuf:"varint,3,opt,name=player_index,json=playerIndex,proto3" json:"player_index,omitempty"`
	Action        GamePlayerActionType   `protobuf:"varint,4,opt,name=action,proto3,enum=multi.GamePlayerActionType" json:"action,omitempty"`
	SlotIndex     int32                  `protobuf:"varint,5,opt,name=slot_index,json=slotIndex,proto3" json:"slot_index,omitempty"`
	Target        int32                  `protobuf:"varint,6,opt,name=target,proto3" json:"target,omitempty"`
	ElementId     string                 `protobuf:"bytes,7,opt,name=element_id,json=elementId,proto3" json:"element_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GameEnqueueActionNotification) Reset() {
	*x = GameEnqueueActionNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GameEnqueueActionNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GameEnqueueActionNotification) ProtoMessage() {}

func (x *GameEnqueueActionNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GameEnqueueActionNotification.ProtoReflect.Descriptor instead.
func (*GameEnqueueActionNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{14}
}

func (x *GameEnqueueActionNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

func (x *GameEnqueueActionNotification) GetPartyIndex() int32 {
	if x != nil {
		return x.PartyIndex
	}
	return 0
}

func (x *GameEnqueueActionNotification) GetPlayerIndex() int32 {
	if x != nil {
		return x.PlayerIndex
	}
	return 0
}

func (x *GameEnqueueActionNotification) GetAction() GamePlayerActionType {
	if x != nil {
		return x.Action
	}
	return GamePlayerActionType_PLAYER_ACTION_TYPE_NONE
}

func (x *GameEnqueueActionNotification) GetSlotIndex() int32 {
	if x != nil {
		return x.SlotIndex
	}
	return 0
}

func (x *GameEnqueueActionNotification) GetTarget() int32 {
	if x != nil {
		return x.Target
	}
	return 0
}

func (x *GameEnqueueActionNotification) GetElementId() string {
	if x != nil {
		return x.ElementId
	}
	return ""
}

type GameDequeueActionNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	GameId        string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	PartyIndex    int32                  `protobuf:"varint,2,opt,name=party_index,json=partyIndex,proto3" json:"party_index,omitempty"`
	PlayerIndex   int32                  `protobuf:"varint,3,opt,name=player_index,json=playerIndex,proto3" json:"player_index,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GameDequeueActionNotification) Reset() {
	*x = GameDequeueActionNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GameDequeueActionNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GameDequeueActionNotification) ProtoMessage() {}

func (x *GameDequeueActionNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GameDequeueActionNotification.ProtoReflect.Descriptor instead.
func (*GameDequeueActionNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{15}
}

func (x *GameDequeueActionNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

func (x *GameDequeueActionNotification) GetPartyIndex() int32 {
	if x != nil {
		return x.PartyIndex
	}
	return 0
}

func (x *GameDequeueActionNotification) GetPlayerIndex() int32 {
	if x != nil {
		return x.PlayerIndex
	}
	return 0
}

type GameLockActionNotification struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	GameId          string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	PartyIndex      int32                  `protobuf:"varint,2,opt,name=party_index,json=partyIndex,proto3" json:"party_index,omitempty"`
	PlayerIndex     int32                  `protobuf:"varint,3,opt,name=player_index,json=playerIndex,proto3" json:"player_index,omitempty"`
	ActionLockIndex int32                  `protobuf:"varint,4,opt,name=action_lock_index,json=actionLockIndex,proto3" json:"action_lock_index,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *GameLockActionNotification) Reset() {
	*x = GameLockActionNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GameLockActionNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GameLockActionNotification) ProtoMessage() {}

func (x *GameLockActionNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GameLockActionNotification.ProtoReflect.Descriptor instead.
func (*GameLockActionNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{16}
}

func (x *GameLockActionNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

func (x *GameLockActionNotification) GetPartyIndex() int32 {
	if x != nil {
		return x.PartyIndex
	}
	return 0
}

func (x *GameLockActionNotification) GetPlayerIndex() int32 {
	if x != nil {
		return x.PlayerIndex
	}
	return 0
}

func (x *GameLockActionNotification) GetActionLockIndex() int32 {
	if x != nil {
		return x.ActionLockIndex
	}
	return 0
}

type GameActionQueueConfirmNotification struct {
	state             protoimpl.MessageState       `protogen:"open.v1"`
	PartyActionQueues []*ProtoGamePartyActionQueue `protobuf:"bytes,1,rep,name=party_action_queues,json=partyActionQueues,proto3" json:"party_action_queues,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *GameActionQueueConfirmNotification) Reset() {
	*x = GameActionQueueConfirmNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GameActionQueueConfirmNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GameActionQueueConfirmNotification) ProtoMessage() {}

func (x *GameActionQueueConfirmNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GameActionQueueConfirmNotification.ProtoReflect.Descriptor instead.
func (*GameActionQueueConfirmNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{17}
}

func (x *GameActionQueueConfirmNotification) GetPartyActionQueues() []*ProtoGamePartyActionQueue {
	if x != nil {
		return x.PartyActionQueues
	}
	return nil
}

type GamePlayerRemoveNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	GameId        string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	PartyIndex    int32                  `protobuf:"varint,2,opt,name=party_index,json=partyIndex,proto3" json:"party_index,omitempty"`
	PlayerIndex   int32                  `protobuf:"varint,3,opt,name=player_index,json=playerIndex,proto3" json:"player_index,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GamePlayerRemoveNotification) Reset() {
	*x = GamePlayerRemoveNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GamePlayerRemoveNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GamePlayerRemoveNotification) ProtoMessage() {}

func (x *GamePlayerRemoveNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GamePlayerRemoveNotification.ProtoReflect.Descriptor instead.
func (*GamePlayerRemoveNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{18}
}

func (x *GamePlayerRemoveNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

func (x *GamePlayerRemoveNotification) GetPartyIndex() int32 {
	if x != nil {
		return x.PartyIndex
	}
	return 0
}

func (x *GamePlayerRemoveNotification) GetPlayerIndex() int32 {
	if x != nil {
		return x.PlayerIndex
	}
	return 0
}

type GamePlayerDisconnectNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	GameId        string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	PartyIndex    int32                  `protobuf:"varint,2,opt,name=party_index,json=partyIndex,proto3" json:"party_index,omitempty"`
	PlayerIndex   int32                  `protobuf:"varint,3,opt,name=player_index,json=playerIndex,proto3" json:"player_index,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GamePlayerDisconnectNotification) Reset() {
	*x = GamePlayerDisconnectNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GamePlayerDisconnectNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GamePlayerDisconnectNotification) ProtoMessage() {}

func (x *GamePlayerDisconnectNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GamePlayerDisconnectNotification.ProtoReflect.Descriptor instead.
func (*GamePlayerDisconnectNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{19}
}

func (x *GamePlayerDisconnectNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

func (x *GamePlayerDisconnectNotification) GetPartyIndex() int32 {
	if x != nil {
		return x.PartyIndex
	}
	return 0
}

func (x *GamePlayerDisconnectNotification) GetPlayerIndex() int32 {
	if x != nil {
		return x.PlayerIndex
	}
	return 0
}

type GamePlayerReconnectNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	GameId        string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	PartyIndex    int32                  `protobuf:"varint,2,opt,name=party_index,json=partyIndex,proto3" json:"party_index,omitempty"`
	PlayerIndex   int32                  `protobuf:"varint,3,opt,name=player_index,json=playerIndex,proto3" json:"player_index,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GamePlayerReconnectNotification) Reset() {
	*x = GamePlayerReconnectNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GamePlayerReconnectNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GamePlayerReconnectNotification) ProtoMessage() {}

func (x *GamePlayerReconnectNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GamePlayerReconnectNotification.ProtoReflect.Descriptor instead.
func (*GamePlayerReconnectNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{20}
}

func (x *GamePlayerReconnectNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

func (x *GamePlayerReconnectNotification) GetPartyIndex() int32 {
	if x != nil {
		return x.PartyIndex
	}
	return 0
}

func (x *GamePlayerReconnectNotification) GetPlayerIndex() int32 {
	if x != nil {
		return x.PlayerIndex
	}
	return 0
}

type GamePlayerDeathNotification struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	GameId        string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	PartyIndex    int32                  `protobuf:"varint,2,opt,name=party_index,json=partyIndex,proto3" json:"party_index,omitempty"`
	PlayerIndex   int32                  `protobuf:"varint,3,opt,name=player_index,json=playerIndex,proto3" json:"player_index,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GamePlayerDeathNotification) Reset() {
	*x = GamePlayerDeathNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GamePlayerDeathNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GamePlayerDeathNotification) ProtoMessage() {}

func (x *GamePlayerDeathNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GamePlayerDeathNotification.ProtoReflect.Descriptor instead.
func (*GamePlayerDeathNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{21}
}

func (x *GamePlayerDeathNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

func (x *GamePlayerDeathNotification) GetPartyIndex() int32 {
	if x != nil {
		return x.PartyIndex
	}
	return 0
}

func (x *GamePlayerDeathNotification) GetPlayerIndex() int32 {
	if x != nil {
		return x.PlayerIndex
	}
	return 0
}

type GamePlayerReviveNotification struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	GameId         string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	PartyIndex     int32                  `protobuf:"varint,2,opt,name=party_index,json=partyIndex,proto3" json:"party_index,omitempty"`
	PlayerIndex    int32                  `protobuf:"varint,3,opt,name=player_index,json=playerIndex,proto3" json:"player_index,omitempty"`
	SourcePlayerId string                 `protobuf:"bytes,4,opt,name=source_player_id,json=sourcePlayerId,proto3" json:"source_player_id,omitempty"` // who revived them; equals the target's own player id on self-revive
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *GamePlayerReviveNotification) Reset() {
	*x = GamePlayerReviveNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GamePlayerReviveNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GamePlayerReviveNotification) ProtoMessage() {}

func (x *GamePlayerReviveNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GamePlayerReviveNotification.ProtoReflect.Descriptor instead.
func (*GamePlayerReviveNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{22}
}

func (x *GamePlayerReviveNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

func (x *GamePlayerReviveNotification) GetPartyIndex() int32 {
	if x != nil {
		return x.PartyIndex
	}
	return 0
}

func (x *GamePlayerReviveNotification) GetPlayerIndex() int32 {
	if x != nil {
		return x.PlayerIndex
	}
	return 0
}

func (x *GamePlayerReviveNotification) GetSourcePlayerId() string {
	if x != nil {
		return x.SourcePlayerId
	}
	return ""
}

type GamePlayerReviveClaimNotification struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	GameId         string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	PartyIndex     int32                  `protobuf:"varint,2,opt,name=party_index,json=partyIndex,proto3" json:"party_index,omitempty"`
	PlayerIndex    int32                  `protobuf:"varint,3,opt,name=player_index,json=playerIndex,proto3" json:"player_index,omitempty"`
	SourcePlayerId string                 `protobuf:"bytes,4,opt,name=source_player_id,json=sourcePlayerId,proto3" json:"source_player_id,omitempty"` // who holds the claim; equals the target's own player id on a self-revive claim
	RemainingMs    int64                  `protobuf:"varint,5,opt,name=remaining_ms,json=remainingMs,proto3" json:"remaining_ms,omitempty"`           // claim window as granted; a refresh re-broadcasts with a fresh window
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *GamePlayerReviveClaimNotification) Reset() {
	*x = GamePlayerReviveClaimNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GamePlayerReviveClaimNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GamePlayerReviveClaimNotification) ProtoMessage() {}

func (x *GamePlayerReviveClaimNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GamePlayerReviveClaimNotification.ProtoReflect.Descriptor instead.
func (*GamePlayerReviveClaimNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{23}
}

func (x *GamePlayerReviveClaimNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

func (x *GamePlayerReviveClaimNotification) GetPartyIndex() int32 {
	if x != nil {
		return x.PartyIndex
	}
	return 0
}

func (x *GamePlayerReviveClaimNotification) GetPlayerIndex() int32 {
	if x != nil {
		return x.PlayerIndex
	}
	return 0
}

func (x *GamePlayerReviveClaimNotification) GetSourcePlayerId() string {
	if x != nil {
		return x.SourcePlayerId
	}
	return ""
}

func (x *GamePlayerReviveClaimNotification) GetRemainingMs() int64 {
	if x != nil {
		return x.RemainingMs
	}
	return 0
}

type GamePlayerReviveClaimExpireNotification struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	GameId         string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	PartyIndex     int32                  `protobuf:"varint,2,opt,name=party_index,json=partyIndex,proto3" json:"party_index,omitempty"`
	PlayerIndex    int32                  `protobuf:"varint,3,opt,name=player_index,json=playerIndex,proto3" json:"player_index,omitempty"`
	SourcePlayerId string                 `protobuf:"bytes,4,opt,name=source_player_id,json=sourcePlayerId,proto3" json:"source_player_id,omitempty"` // whose claim lapsed without a revive; the corpse is claimable again
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *GamePlayerReviveClaimExpireNotification) Reset() {
	*x = GamePlayerReviveClaimExpireNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GamePlayerReviveClaimExpireNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GamePlayerReviveClaimExpireNotification) ProtoMessage() {}

func (x *GamePlayerReviveClaimExpireNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GamePlayerReviveClaimExpireNotification.ProtoReflect.Descriptor instead.
func (*GamePlayerReviveClaimExpireNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{24}
}

func (x *GamePlayerReviveClaimExpireNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

func (x *GamePlayerReviveClaimExpireNotification) GetPartyIndex() int32 {
	if x != nil {
		return x.PartyIndex
	}
	return 0
}

func (x *GamePlayerReviveClaimExpireNotification) GetPlayerIndex() int32 {
	if x != nil {
		return x.PlayerIndex
	}
	return 0
}

func (x *GamePlayerReviveClaimExpireNotification) GetSourcePlayerId() string {
	if x != nil {
		return x.SourcePlayerId
	}
	return ""
}

type GameSyncNotification struct {
	state   protoimpl.MessageState `protogen:"open.v1"`
	GameId  string                 `protobuf:"bytes,1,opt,name=game_id,json=gameId,proto3" json:"game_id,omitempty"`
	Parties []*ProtoGameSyncParty  `protobuf:"bytes,2,rep,name=parties,proto3" json:"parties,omitempty"`
	// ----- optional, see notes -----
	Phase           GameSyncPhase       `protobuf:"varint,3,opt,name=phase,proto3,enum=multi.GameSyncPhase" json:"phase,omitempty"`                     // insurance vs. the detect-latency window
	TurnRemainingMs int64               `protobuf:"varint,4,opt,name=turn_remaining_ms,json=turnRemainingMs,proto3" json:"turn_remaining_ms,omitempty"` // player-turn countdown; 0 if untimed/not player turn
	Enemies         []*ProtoGameEnemyHP `protobuf:"bytes,5,rep,name=enemies,proto3" json:"enemies,omitempty"`                                           // latest resolved consensus; empty if none yet
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *GameSyncNotification) Reset() {
	*x = GameSyncNotification{}
	mi := &file_protomulti_multi_notification_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GameSyncNotification) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GameSyncNotification) ProtoMessage() {}

func (x *GameSyncNotification) ProtoReflect() protoreflect.Message {
	mi := &file_protomulti_multi_notification_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GameSyncNotification.ProtoReflect.Descriptor instead.
func (*GameSyncNotification) Descriptor() ([]byte, []int) {
	return file_protomulti_multi_notification_proto_rawDescGZIP(), []int{25}
}

func (x *GameSyncNotification) GetGameId() string {
	if x != nil {
		return x.GameId
	}
	return ""
}

func (x *GameSyncNotification) GetParties() []*ProtoGameSyncParty {
	if x != nil {
		return x.Parties
	}
	return nil
}

func (x *GameSyncNotification) GetPhase() GameSyncPhase {
	if x != nil {
		return x.Phase
	}
	return GameSyncPhase_GAME_SYNC_PHASE_UNKNOWN
}

func (x *GameSyncNotification) GetTurnRemainingMs() int64 {
	if x != nil {
		return x.TurnRemainingMs
	}
	return 0
}

func (x *GameSyncNotification) GetEnemies() []*ProtoGameEnemyHP {
	if x != nil {
		return x.Enemies
	}
	return nil
}

var File_protomulti_multi_notification_proto protoreflect.FileDescriptor

const file_protomulti_multi_notification_proto_rawDesc = "" +
	"\n" +
	"#protomulti/multi.notification.proto\x12\x05multi\x1a\x16protomulti/multi.proto\"4\n" +
	"\x17LobbyCancelNotification\x12\x19\n" +
	"\blobby_id\x18\x01 \x01(\tR\alobbyId\"L\n" +
	"\x16LobbyStartNotification\x12\x19\n" +
	"\blobby_id\x18\x01 \x01(\tR\alobbyId\x12\x17\n" +
	"\agame_id\x18\x02 \x01(\tR\x06gameId\"3\n" +
	"\x16LobbyReadyNotification\x12\x19\n" +
	"\blobby_id\x18\x01 \x01(\tR\alobbyId\"\xc6\x01\n" +
	"\x1bParticipantJoinNotification\x12\x19\n" +
	"\blobby_id\x18\x01 \x01(\tR\alobbyId\x12\x1b\n" +
	"\tplayer_id\x18\x02 \x01(\tR\bplayerId\x12\x1d\n" +
	"\n" +
	"deck_index\x18\x03 \x01(\x05R\tdeckIndex\x12\x1f\n" +
	"\vplayer_slot\x18\x04 \x01(\x05R\n" +
	"playerSlot\x12/\n" +
	"\x06player\x18\x05 \x01(\v2\x17.multi.ProtoLobbyPlayerR\x06player\"w\n" +
	"\x1cParticipantLeaveNotification\x12\x19\n" +
	"\blobby_id\x18\x01 \x01(\tR\alobbyId\x12\x1b\n" +
	"\tplayer_id\x18\x02 \x01(\tR\bplayerId\x12\x1f\n" +
	"\vplayer_slot\x18\x03 \x01(\x05R\n" +
	"playerSlot\"Z\n" +
	"\x1cParticipantReadyNotification\x12\x19\n" +
	"\blobby_id\x18\x01 \x01(\tR\alobbyId\x12\x1f\n" +
	"\vplayer_slot\x18\x03 \x01(\x05R\n" +
	"playerSlot\"Z\n" +
	"\x1eParticipantUnreadyNotification\x12\x19\n" +
	"\blobby_id\x18\x01 \x01(\tR\alobbyId\x12\x1d\n" +
	"\n" +
	"party_slot\x18\x02 \x01(\x05R\tpartySlot\"\x90\x01\n" +
	"!ParticipantDeckChangeNotification\x12\x19\n" +
	"\blobby_id\x18\x01 \x01(\tR\alobbyId\x12\x1f\n" +
	"\vplayer_slot\x18\x03 \x01(\x05R\n" +
	"playerSlot\x12/\n" +
	"\x06player\x18\x04 \x01(\v2\x17.multi.ProtoLobbyPlayerR\x06player\"l\n" +
	"\x15StampSendNotification\x12\x19\n" +
	"\blobby_id\x18\x01 \x01(\tR\alobbyId\x12\x19\n" +
	"\bstamp_id\x18\x02 \x01(\tR\astampId\x12\x1d\n" +
	"\n" +
	"party_slot\x18\x03 \x01(\x05R\tpartySlot\"0\n" +
	"\x15GameStartNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\".\n" +
	"\x13GameEndNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\"z\n" +
	"\x1bGamePlayerReadyNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\x12\x1f\n" +
	"\vparty_index\x18\x02 \x01(\x05R\n" +
	"partyIndex\x12!\n" +
	"\fplayer_index\x18\x03 \x01(\x05R\vplayerIndex\"0\n" +
	"\x15GameReadyNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\"d\n" +
	"\x16GameHPSyncNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\x121\n" +
	"\aenemies\x18\x02 \x03(\v2\x17.multi.ProtoGameEnemyHPR\aenemies\"\x87\x02\n" +
	"\x1dGameEnqueueActionNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\x12\x1f\n" +
	"\vparty_index\x18\x02 \x01(\x05R\n" +
	"partyIndex\x12!\n" +
	"\fplayer_index\x18\x03 \x01(\x05R\vplayerIndex\x123\n" +
	"\x06action\x18\x04 \x01(\x0e2\x1b.multi.GamePlayerActionTypeR\x06action\x12\x1d\n" +
	"\n" +
	"slot_index\x18\x05 \x01(\x05R\tslotIndex\x12\x16\n" +
	"\x06target\x18\x06 \x01(\x05R\x06target\x12\x1d\n" +
	"\n" +
	"element_id\x18\a \x01(\tR\telementId\"|\n" +
	"\x1dGameDequeueActionNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\x12\x1f\n" +
	"\vparty_index\x18\x02 \x01(\x05R\n" +
	"partyIndex\x12!\n" +
	"\fplayer_index\x18\x03 \x01(\x05R\vplayerIndex\"\xa5\x01\n" +
	"\x1aGameLockActionNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\x12\x1f\n" +
	"\vparty_index\x18\x02 \x01(\x05R\n" +
	"partyIndex\x12!\n" +
	"\fplayer_index\x18\x03 \x01(\x05R\vplayerIndex\x12*\n" +
	"\x11action_lock_index\x18\x04 \x01(\x05R\x0factionLockIndex\"v\n" +
	"\"GameActionQueueConfirmNotification\x12P\n" +
	"\x13party_action_queues\x18\x01 \x03(\v2 .multi.ProtoGamePartyActionQueueR\x11partyActionQueues\"{\n" +
	"\x1cGamePlayerRemoveNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\x12\x1f\n" +
	"\vparty_index\x18\x02 \x01(\x05R\n" +
	"partyIndex\x12!\n" +
	"\fplayer_index\x18\x03 \x01(\x05R\vplayerIndex\"\x7f\n" +
	" GamePlayerDisconnectNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\x12\x1f\n" +
	"\vparty_index\x18\x02 \x01(\x05R\n" +
	"partyIndex\x12!\n" +
	"\fplayer_index\x18\x03 \x01(\x05R\vplayerIndex\"~\n" +
	"\x1fGamePlayerReconnectNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\x12\x1f\n" +
	"\vparty_index\x18\x02 \x01(\x05R\n" +
	"partyIndex\x12!\n" +
	"\fplayer_index\x18\x03 \x01(\x05R\vplayerIndex\"z\n" +
	"\x1bGamePlayerDeathNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\x12\x1f\n" +
	"\vparty_index\x18\x02 \x01(\x05R\n" +
	"partyIndex\x12!\n" +
	"\fplayer_index\x18\x03 \x01(\x05R\vplayerIndex\"\xa5\x01\n" +
	"\x1cGamePlayerReviveNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\x12\x1f\n" +
	"\vparty_index\x18\x02 \x01(\x05R\n" +
	"partyIndex\x12!\n" +
	"\fplayer_index\x18\x03 \x01(\x05R\vplayerIndex\x12(\n" +
	"\x10source_player_id\x18\x04 \x01(\tR\x0esourcePlayerId\"\xcd\x01\n" +
	"!GamePlayerReviveClaimNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\x12\x1f\n" +
	"\vparty_index\x18\x02 \x01(\x05R\n" +
	"partyIndex\x12!\n" +
	"\fplayer_index\x18\x03 \x01(\x05R\vplayerIndex\x12(\n" +
	"\x10source_player_id\x18\x04 \x01(\tR\x0esourcePlayerId\x12!\n" +
	"\fremaining_ms\x18\x05 \x01(\x03R\vremainingMs\"\xb0\x01\n" +
	"'GamePlayerReviveClaimExpireNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\x12\x1f\n" +
	"\vparty_index\x18\x02 \x01(\x05R\n" +
	"partyIndex\x12!\n" +
	"\fplayer_index\x18\x03 \x01(\x05R\vplayerIndex\x12(\n" +
	"\x10source_player_id\x18\x04 \x01(\tR\x0esourcePlayerId\"\xef\x01\n" +
	"\x14GameSyncNotification\x12\x17\n" +
	"\agame_id\x18\x01 \x01(\tR\x06gameId\x123\n" +
	"\aparties\x18\x02 \x03(\v2\x19.multi.ProtoGameSyncPartyR\aparties\x12*\n" +
	"\x05phase\x18\x03 \x01(\x0e2\x14.multi.GameSyncPhaseR\x05phase\x12*\n" +
	"\x11turn_remaining_ms\x18\x04 \x01(\x03R\x0fturnRemainingMs\x121\n" +
	"\aenemies\x18\x05 \x03(\v2\x17.multi.ProtoGameEnemyHPR\aenemies*\xdc\x02\n" +
	"\x1aMultiLobbyNotificationType\x12\x15\n" +
	"\x11LOBBY_NOTIFY_NONE\x10\x00\x12!\n" +
	"\x1dLOBBY_NOTIFY_PARTICIPANT_JOIN\x10d\x12#\n" +
	"\x1eLOBBY_NOTIFY_PARTICIPANT_LEAVE\x10\xc8\x01\x12#\n" +
	"\x1eLOBBY_NOTIFY_PARTICIPANT_READY\x10\xac\x02\x12%\n" +
	" LOBBY_NOTIFY_PARTICIPANT_UNREADY\x10\x90\x03\x12\x1c\n" +
	"\x17LOBBY_NOTIFY_STAMP_SEND\x10\xf4\x03\x12)\n" +
	"$LOBBY_NOTIFY_PARTICIPANT_DECK_CHANGE\x10\xd8\x04\x12\x18\n" +
	"\x13LOBBY_NOTIFY_CANCEL\x10\xbc\x05\x12\x17\n" +
	"\x12LOBBY_NOTIFY_START\x10\xa0\x06\x12\x17\n" +
	"\x12LOBBY_NOTIFY_READY\x10\x84\a*\xc9\x04\n" +
	"\x19MultiGameNotificationType\x12\x14\n" +
	"\x10GAME_NOTIFY_NONE\x10\x00\x12\x15\n" +
	"\x11GAME_NOTIFY_START\x10d\x12\x16\n" +
	"\x11GAME_NOTIFY_READY\x10\xc8\x01\x12\x1d\n" +
	"\x18GAME_NOTIFY_PLAYER_READY\x10\xfa\x01\x12\x1e\n" +
	"\x19GAME_NOTIFY_PLAYER_REMOVE\x10\x93\x02\x12\x1f\n" +
	"\x1aGAME_NOTIFY_ENQUEUE_ACTION\x10\xdc\v\x12\x1f\n" +
	"\x1aGAME_NOTIFY_DEQUEUE_ACTION\x10\xc0\f\x12\x1c\n" +
	"\x17GAME_NOTIFY_LOCK_ACTION\x10\xa4\r\x12\x1e\n" +
	"\x19GAME_NOTIFY_QUEUE_CONFIRM\x10\x88\x0e\x12\x1d\n" +
	"\x18GAME_NOTIFY_HP_CONSENSUS\x10\xd0\x0f\x12\"\n" +
	"\x1dGAME_NOTIFY_PLAYER_DISCONNECT\x10\xb4\x10\x12!\n" +
	"\x1cGAME_NOTIFY_PLAYER_RECONNECT\x10\x98\x11\x12\x1a\n" +
	"\x15GAME_NOTIFY_GAME_SYNC\x10\xfc\x11\x12\x1d\n" +
	"\x18GAME_NOTIFY_PLAYER_DEATH\x10\xe0\x12\x12\x1e\n" +
	"\x19GAME_NOTIFY_PLAYER_REVIVE\x10\xc4\x13\x12$\n" +
	"\x1fGAME_NOTIFY_PLAYER_REVIVE_CLAIM\x10\xa8\x14\x12+\n" +
	"&GAME_NOTIFY_PLAYER_REVIVE_CLAIM_EXPIRE\x10\x8c\x15\x12\x14\n" +
	"\x0fGAME_NOTIFY_END\x10\xb8EBMZ6github.com/justjack1521/mevium/pkg/genproto/protomulti\xaa\x02\x12Mobius.Proto.Multib\x06proto3"

var (
	file_protomulti_multi_notification_proto_rawDescOnce sync.Once
	file_protomulti_multi_notification_proto_rawDescData []byte
)

func file_protomulti_multi_notification_proto_rawDescGZIP() []byte {
	file_protomulti_multi_notification_proto_rawDescOnce.Do(func() {
		file_protomulti_multi_notification_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_protomulti_multi_notification_proto_rawDesc), len(file_protomulti_multi_notification_proto_rawDesc)))
	})
	return file_protomulti_multi_notification_proto_rawDescData
}

var file_protomulti_multi_notification_proto_enumTypes = make([]protoimpl.EnumInfo, 2)
var file_protomulti_multi_notification_proto_msgTypes = make([]protoimpl.MessageInfo, 26)
var file_protomulti_multi_notification_proto_goTypes = []any{
	(MultiLobbyNotificationType)(0),                 // 0: multi.MultiLobbyNotificationType
	(MultiGameNotificationType)(0),                  // 1: multi.MultiGameNotificationType
	(*LobbyCancelNotification)(nil),                 // 2: multi.LobbyCancelNotification
	(*LobbyStartNotification)(nil),                  // 3: multi.LobbyStartNotification
	(*LobbyReadyNotification)(nil),                  // 4: multi.LobbyReadyNotification
	(*ParticipantJoinNotification)(nil),             // 5: multi.ParticipantJoinNotification
	(*ParticipantLeaveNotification)(nil),            // 6: multi.ParticipantLeaveNotification
	(*ParticipantReadyNotification)(nil),            // 7: multi.ParticipantReadyNotification
	(*ParticipantUnreadyNotification)(nil),          // 8: multi.ParticipantUnreadyNotification
	(*ParticipantDeckChangeNotification)(nil),       // 9: multi.ParticipantDeckChangeNotification
	(*StampSendNotification)(nil),                   // 10: multi.StampSendNotification
	(*GameStartNotification)(nil),                   // 11: multi.GameStartNotification
	(*GameEndNotification)(nil),                     // 12: multi.GameEndNotification
	(*GamePlayerReadyNotification)(nil),             // 13: multi.GamePlayerReadyNotification
	(*GameReadyNotification)(nil),                   // 14: multi.GameReadyNotification
	(*GameHPSyncNotification)(nil),                  // 15: multi.GameHPSyncNotification
	(*GameEnqueueActionNotification)(nil),           // 16: multi.GameEnqueueActionNotification
	(*GameDequeueActionNotification)(nil),           // 17: multi.GameDequeueActionNotification
	(*GameLockActionNotification)(nil),              // 18: multi.GameLockActionNotification
	(*GameActionQueueConfirmNotification)(nil),      // 19: multi.GameActionQueueConfirmNotification
	(*GamePlayerRemoveNotification)(nil),            // 20: multi.GamePlayerRemoveNotification
	(*GamePlayerDisconnectNotification)(nil),        // 21: multi.GamePlayerDisconnectNotification
	(*GamePlayerReconnectNotification)(nil),         // 22: multi.GamePlayerReconnectNotification
	(*GamePlayerDeathNotification)(nil),             // 23: multi.GamePlayerDeathNotification
	(*GamePlayerReviveNotification)(nil),            // 24: multi.GamePlayerReviveNotification
	(*GamePlayerReviveClaimNotification)(nil),       // 25: multi.GamePlayerReviveClaimNotification
	(*GamePlayerReviveClaimExpireNotification)(nil), // 26: multi.GamePlayerReviveClaimExpireNotification
	(*GameSyncNotification)(nil),                    // 27: multi.GameSyncNotification
	(*ProtoLobbyPlayer)(nil),                        // 28: multi.ProtoLobbyPlayer
	(*ProtoGameEnemyHP)(nil),                        // 29: multi.ProtoGameEnemyHP
	(GamePlayerActionType)(0),                       // 30: multi.GamePlayerActionType
	(*ProtoGamePartyActionQueue)(nil),               // 31: multi.ProtoGamePartyActionQueue
	(*ProtoGameSyncParty)(nil),                      // 32: multi.ProtoGameSyncParty
	(GameSyncPhase)(0),                              // 33: multi.GameSyncPhase
}
var file_protomulti_multi_notification_proto_depIdxs = []int32{
	28, // 0: multi.ParticipantJoinNotification.player:type_name -> multi.ProtoLobbyPlayer
	28, // 1: multi.ParticipantDeckChangeNotification.player:type_name -> multi.ProtoLobbyPlayer
	29, // 2: multi.GameHPSyncNotification.enemies:type_name -> multi.ProtoGameEnemyHP
	30, // 3: multi.GameEnqueueActionNotification.action:type_name -> multi.GamePlayerActionType
	31, // 4: multi.GameActionQueueConfirmNotification.party_action_queues:type_name -> multi.ProtoGamePartyActionQueue
	32, // 5: multi.GameSyncNotification.parties:type_name -> multi.ProtoGameSyncParty
	33, // 6: multi.GameSyncNotification.phase:type_name -> multi.GameSyncPhase
	29, // 7: multi.GameSyncNotification.enemies:type_name -> multi.ProtoGameEnemyHP
	8,  // [8:8] is the sub-list for method output_type
	8,  // [8:8] is the sub-list for method input_type
	8,  // [8:8] is the sub-list for extension type_name
	8,  // [8:8] is the sub-list for extension extendee
	0,  // [0:8] is the sub-list for field type_name
}

func init() { file_protomulti_multi_notification_proto_init() }
func file_protomulti_multi_notification_proto_init() {
	if File_protomulti_multi_notification_proto != nil {
		return
	}
	file_protomulti_multi_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_protomulti_multi_notification_proto_rawDesc), len(file_protomulti_multi_notification_proto_rawDesc)),
			NumEnums:      2,
			NumMessages:   26,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_protomulti_multi_notification_proto_goTypes,
		DependencyIndexes: file_protomulti_multi_notification_proto_depIdxs,
		EnumInfos:         file_protomulti_multi_notification_proto_enumTypes,
		MessageInfos:      file_protomulti_multi_notification_proto_msgTypes,
	}.Build()
	File_protomulti_multi_notification_proto = out.File
	file_protomulti_multi_notification_proto_goTypes = nil
	file_protomulti_multi_notification_proto_depIdxs = nil
}
